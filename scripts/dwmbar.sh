...
cpu_info() {
	...
	printf "^c#f6edf6^ ^b#400040^ "
	printf "^c#f6edf6^ ^b#400040^ $cpu_load"
}

## Memory
memory() {
	printf "^c#f6edf6^^b#400040^   $(free -h | awk '/^Mem/ { print $3 }' | sed s/i//g) "
}

## Wi-fi
wlan() {
	case "$(cat /sys/class/net/w*/operstate 2>/dev/null)" in
		up) printf "  ^d^%s^b#400040^ Connected " ;;
		down) printf " 睊 ^d^%s^b#400040^ Disconnected " ;;
	esac
}
...
updates() {
	...
	if [ -z "$updates" ]; then
		printf "^c#f6edf6^^b#FFC300^  Updated"
	else
		printf "^c#f6edf6^^b#400040^ ^b#400040^ $updates "" updates"
		printf "^c#f6edf6^"
	fi
}
